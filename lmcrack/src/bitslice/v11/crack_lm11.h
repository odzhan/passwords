/** Fixed printable-LM-alphabet incremental bitsliced DES worker. */
...
static bool crack_lm11(void *param)
{
    crack_opt_t *c=(crack_opt_t*)param;
    alignas(64) v11_bitslice_counter counter;
    alignas(64) bs_vec password[V11_LM_KEY_PLANES];
    ...
    if (c->alpha_len!=(int)V11_ALPHABET_LENGTH ||
        memcmp(c->alphabet,V11_ALPHABET,V11_ALPHABET_LENGTH+1U)!=0 ||
        candidate_count==0 || c->end_cbn<c->start_cbn ||
        candidate_count!=c->end_cbn-c->start_cbn ||
        !fixed_prepare_target_state(c->hash.b,&target) ||
        !v11_bs_counter_init(&counter,c->start_cbn,candidate_count)) return false;
    ...
    while (!c->stopped.load(std::memory_order_relaxed)) {
      ...
      if (count==0 || !v11_bs_make_key_planes(&counter,password) ||
          !bs_copy_lm_plaintext_state(&state)) return false;
      fixed_bs_des_rounds(state.left,state.right,password);
      const bs_vec matches=fixed_match_target_state(
        state.left,state.right,&target,count);
      const size_t lane=bs_first_match_lane(matches,count);
      c->complete.fetch_add((uint64_t)count,std::memory_order_relaxed);
      const uint64_t remaining=c->total_cbn.fetch_sub(
        (uint64_t)count,std::memory_order_relaxed);
      if (lane<count) {
        uint8_t digits[V11_MAX_PASSWORD_LENGTH]; unsigned int length;
        if (!v11_cbn_to_digits(batch_start+(uint64_t)lane,digits,&length))
          return false;
      ...
      if (!v11_bs_counter_advance(&counter)) return false;
    }
    return false;
}
