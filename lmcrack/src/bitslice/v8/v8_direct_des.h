 ...
 * v8 select round keys without materializing a 16 x 48 vector schedule.
 */
static const uint8_t v8_round_key_plane[BS_DES_ROUNDS][BS_ROUND_KEY_PLANES]={
...
static inline void v8_bs_des_f(const bs_vec right[32],
                               const bs_vec password[V8_LM_KEY_PLANES],
                               bs_vec output[32])
{
    ...
    for (unsigned int box=0;box<8;box++) {
      for (unsigned int bit=0;bit<6;bit++) {
        ...
        input[bit]=bs_xor(right[bs_des_e[expanded]-1U],
                          password[v8_round_key_plane[Round][expanded]]);
      }
...
{
    bs_vec f[32];
    v8_bs_des_f<Round>(source,password,f);
    for (unsigned int bit=0;bit<32;bit++)
...
static inline void v8_bs_des_rounds(bs_vec left[32],bs_vec right[32],
                                    const bs_vec password[V8_LM_KEY_PLANES])
{
    v8_bs_apply_round<0>(left,right,password);
    v8_bs_apply_round<1>(right,left,password);
    v8_bs_apply_round<2>(left,right,password);
    v8_bs_apply_round<3>(right,left,password);
    v8_bs_apply_round<4>(left,right,password);
    v8_bs_apply_round<5>(right,left,password);
    v8_bs_apply_round<6>(left,right,password);
    v8_bs_apply_round<7>(right,left,password);
    v8_bs_apply_round<8>(left,right,password);
    v8_bs_apply_round<9>(right,left,password);
    v8_bs_apply_round<10>(left,right,password);
    v8_bs_apply_round<11>(right,left,password);
    v8_bs_apply_round<12>(left,right,password);
    v8_bs_apply_round<13>(right,left,password);
    v8_bs_apply_round<14>(left,right,password);
    v8_bs_apply_round<15>(right,left,password);
}
