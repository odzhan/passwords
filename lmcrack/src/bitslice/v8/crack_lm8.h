/** Uppercase-only incremental bitsliced DES cracking worker. */

#include "bitslice_batch.h"
#include "v8_direct_des.h"
#include "v8_target.h"

static bool crack_lm8(void *param)
{
    crack_opt_t *c=(crack_opt_t*)param;
    alignas(64) v8_bitslice_counter counter;
    alignas(64) bs_vec password[V8_LM_KEY_PLANES];
    alignas(64) bs_block_state state;
    v8_target_state target;
    uint64_t candidate_count=c->total_cbn.load(std::memory_order_relaxed);

    if (c->alpha_len!=(int)V8_ALPHABET_LENGTH ||
        memcmp(c->alphabet,V8_ALPHABET,V8_ALPHABET_LENGTH+1U)!=0 ||
        candidate_count==0 ||
        c->end_cbn<c->start_cbn ||
        candidate_count!=c->end_cbn-c->start_cbn ||
        !v8_prepare_target_state(c->hash.b,&target) ||
        !v8_bs_counter_init(&counter,c->start_cbn,candidate_count))
      return false;

    while (!c->stopped.load(std::memory_order_relaxed)) {
      size_t count=counter.count;
      uint64_t batch_start=counter.current_cbn;
      if (count==0 || !v8_bs_make_key_planes(&counter,password) ||
          !bs_copy_lm_plaintext_state(&state)) return false;

      v8_bs_des_rounds(state.left,state.right,password);
      bs_vec matches=v8_match_target_state(state.left,state.right,&target,count);
      ...
      if (lane<count) {
        ...
        unsigned int length;
        if (!v8_cbn_to_digits(batch_start+(uint64_t)lane,digits,&length))
          return false;
      ...
      if (!v8_bs_counter_advance(&counter)) return false;
    }
    return false;
}
