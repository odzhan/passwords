...
#if defined(LMCRACK_PAIR256)
#if defined(_MSC_VER)
#define DES_ALIGN64 __declspec(align(64))
#elif defined(__GNUC__) || defined(__clang__)
#define DES_ALIGN64 __attribute__((aligned(64)))
#else
#define DES_ALIGN64
#endif

/*
 * Historical pair-merged layout.  The 0xfcfc mask contains two 6-bit
 * indexes separated by two gap bits.  Shifting it by two retains those
 * gaps and produces a 14-bit index, hence 16,384 words (64 KiB) per table.
 */
DES_ALIGN64 static DES_LONG des_pair_T13[16384];
DES_ALIGN64 static DES_LONG des_pair_T24[16384];
DES_ALIGN64 static DES_LONG des_pair_T57[16384];
DES_ALIGN64 static DES_LONG des_pair_T68[16384];

void DES_pair256_init(void)
{
    unsigned int i;

    for (i=0; i<16384; i++) {
        unsigned int lo = i & 0x3fU;
        unsigned int hi = (i >> 8) & 0x3fU;

        des_pair_T13[i] = des_SPtrans[0][lo] ^ des_SPtrans[2][hi];
        des_pair_T24[i] = des_SPtrans[1][lo] ^ des_SPtrans[3][hi];
        des_pair_T57[i] = des_SPtrans[4][lo] ^ des_SPtrans[6][hi];
        des_pair_T68[i] = des_SPtrans[5][lo] ^ des_SPtrans[7][hi];
    }
}
...
/* Dense pair-merged layout: two 6-bit S-box indexes form one 12-bit index. */
DES_ALIGN64 static DES_LONG des_pair64_T13[4096];
DES_ALIGN64 static DES_LONG des_pair64_T24[4096];
DES_ALIGN64 static DES_LONG des_pair64_T57[4096];
DES_ALIGN64 static DES_LONG des_pair64_T68[4096];

void DES_pair64_init(void)
{
    unsigned int i;

    for (i=0; i<4096; i++) {
        unsigned int lo = i & 0x3fU;
        unsigned int hi = (i >> 6) & 0x3fU;

        des_pair64_T13[i] = des_SPtrans[0][lo] ^ des_SPtrans[2][hi];
        des_pair64_T24[i] = des_SPtrans[1][lo] ^ des_SPtrans[3][hi];
        des_pair64_T57[i] = des_SPtrans[4][lo] ^ des_SPtrans[6][hi];
        des_pair64_T68[i] = des_SPtrans[5][lo] ^ des_SPtrans[7][hi];
    }
}
...
{
#if defined(LMCRACK_NEON)
    uint8_t *d = (uint8_t*)ks;
    uint8_t *k = (uint8_t*)key;
    size_t i, j;

    {
      const uint8_t *s = (const uint8_t*)&ks_tbl[0][k[0]];
      for (j=0; j<sizeof(*ks); j+=16)
        vst1q_u8(d+j, vld1q_u8(s+j));
    }

    for (i=1; i<7; i++) {
      const uint8_t *s = (const uint8_t*)&ks_tbl[i][k[i]];
      for (j=0; j<sizeof(*ks); j+=16)
        vst1q_u8(d+j, vorrq_u8(vld1q_u8(d+j), vld1q_u8(s+j)));
    }
#else
    uint32_t *s, *d;
    uint8_t  *k=(uint8_t*)key;
    ...
    // zero initialize
    for(i=0; i<sizeof(*ks)/sizeof(*d); i++)
      d[i]=0;
    
    // for each byte of a 56-bit key
    for(i=0; i<7; i++) {
      // get a key schedule
      s = (uint32_t*)&ks_tbl[i][k[i]];
      
      // perform a bitwise OR
      for(j=0; j<sizeof(*ks)/sizeof(*d); j++)
        d[j] |= s[j];
    }
#endif
}
